...
// Main route handler - catches all paths
app.get('*', (req, res) => {
  ...
  // Construct the original CBC URL
  const cbcUrl = `https://www.cbc.ca${urlPath}${req.url.includes('?') ? '?' + req.url.split('?')[1] : ''}`;
